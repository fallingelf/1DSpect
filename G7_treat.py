...
def overlylist(raw_frame):
    object_frame=[raw_frame[j] for j in range(len(raw_frame)) if raw_frame[j][-1]=='o']
    j=0;listoverly=[];
    while j < len(object_frame):
        suboverly=[];suboverly.append(object_frame[0]);
        for k in range(len(object_frame)-1):
            if suboverly[0][1]==object_frame[k+1][1] and suboverly[0][4:]==object_frame[k+1][4:]:
                suboverly.append(object_frame[k+1]);
        [object_frame.remove(suboverly[k]) for k in range(len(suboverly))]  
        if len(suboverly) > 1:
            listoverly.append(suboverly)
        j+=1;
    return listoverly


def mean_jd_time(obs_time_jd):
    from astropy.time import Time
    t=Time(obs_time_jd,format='jd',scale='utc')
    mean_jd=mean(t.jd);
    mean_date_time=Time(mean(t.jd),format='jd',scale='utc').isot
    return mean_jd,mean_date_time
...
def check_log(path_log,obs_date,path_std):
    ...
    error_log='';standstars=[];data_list=[];fear_list=[];num_fear=0;flag_list=[]
    for j in range(len(txt_log)):      
        if obs_date == txt_log[j][:8]:   #search for list of information of files; the list contains data.
            ...
            if txt_log[j][1].lower().replace('/','')=='bias':          #bias                
                if len(txt_log[j])==4:
                    data_list.append(txt_log[j]);flag_list.append('bias')       #data inf
                    continue
                else:
            ...
            if txt_log[j][1].lower().replace('/','')=='flat':          #flat 
                if len(txt_log[j])==6:
                    data_list.append(txt_log[j]);flag_list.append('flat')        #data inf               
                    continue
                else:
            ...
            if txt_log[j][-1].lower().replace('/','')=='o':         #object 
                if len(txt_log[j])==9:   
                    data_list.append(txt_log[j])        #data inf                    
                    continue
                else:
            ...
            if txt_log[j][-1].lower().replace('/','')=='c':         #companion
                if len(txt_log[j])==9:    
                    ...
                    continue
    for j in range(len(fear_list)):
        if fear_list[j][3]=='300' and num_fear==0:          #append only one FeAr frame
            num_fear=1;data_list.append(fear_list[j]);flag_list.append('FeAr')
    if len(standstars) != 0:   
    ...
    if 'bias' not in set(flag_list):
        error_log=error_log+('No containing bias frame' +'\n')
    if 'flat' not in set(flag_list):
        error_log=error_log+('No containing flat frame' +'\n')
    if 'FeAr' not in set(flag_list):
        error_log=error_log+('No containing FeAr frame' +'\n')
    if len(error_log)!=0:
        ...
        flag=raw_input('Enter [Y]/N to continue\n');rewind=''
        while rewind!='NEXT':
            ...
            if flag.lower()=='n':
                ...
                print('------------------------------------------------')
                flag=raw_input('please enter [Y]/N to continue\n');rewind=''
    return data_list
...
data_list=check_log(PATH_log,OBS_DATE,Path_Folder_script)

inf_file=[]
for j in range(len(data_list)):
    inf_file.append(data_list[j])   
    if len(inf_file[-1])==4:                     #bias                
        ...
        expand_inf_file(inf_file)
        continue
    else:                           #use error to drop out forcely
        print('>>>>>>>>>>>>>>>>>>>LOG&ERROR!<<<<<<<<<<<<<<<<<<<<');print(1/0); 
...
iraf.images.immatch();
list_super=overlylist(inf_file)
for j in range(len(list_super)):
    jd_list=[fits.getheader(list_super[j][k][0], 0)['JD'] for k in range(len(list_super[j]))]
    sup_jd,sup_date_time=mean_jd_time(jd_list)
    sub_file_name=','.join([list_super[j][k][0] for k in range(len(list_super[j]))])
    sup_name=list_super[j][0][0][:8]+list_super[j][0][1]+'.fit'
    iraf.images.immatch.imcomb(sub_file_name,sup_name,rdnoise=Ron,gain=Gain,scale='exposure',expname='exposure',weight='exposure')
    iraf.hedit(sup_name,'JD',str(sup_jd),add=1,ver=0,up=1)
    iraf.hedit(sup_name,'DATE-OBS',sup_date_time,add=1,ver=0,up=1)    
    list_super[j].append([sup_name]+list_super[j][0][1:])
    inf_file.append(list_super[j][-1])
...
iraf.noao.imred();iraf.noao.imred.ccdred();iraf.unlearn('ccdproc');
...
for j in range(len(file_flat)):
    iraf.noao.twodspec.longslit.response(file_flat[j],file_flat[j],'re'+file_flat[j],interactive=0,o=10)
    iraf.noao.twodspec.longslit.illumination('re'+file_flat[j],'il'+file_flat[j],interactive=0,nbins=10,o=10)
...
[iraf.lacos_im(image_spectrum[j],image_spectrum[j].replace('.fit','_clean.fit'),image_spectrum[j].replace('.fit','_clean.pl'),gain=Gain,readn=Ron,sigclip=4,sigfrac=2,niter=3) for j in range(len(image_spectrum))]
pl_files=find_files(Path_Folder_Obj,'_clean.pl');[os.remove(pl_files[j]) for j in range(len(pl_files))];
...
while rewind !='w':
    try:
        rewind='w'
        iraf.noao.twodspec.longslit.splot('*ms.fits');  #inspect the result and remove cosmic rays manually
        continueword = raw_input('Are you sure have finished replacing files contaning cosmic-ray with new files? Enter \'Y\' to continue.');
...
#identify_fwidth=12
#iraf.noao.twod.longs.identify(lamp_files[id_num],coord='linelists$fear.dat',match=10,fwidth=identify_fwidth,nite=1)
#--------------------------------------------------------------------------------------------------------
...
iraf.noao.imred.kpnoslit.dispcor('@mslist','dp_//@mslist',glob=0)
...
if stand_names!=[]:
    ...
    distance_matrix=zeros((len(object_files),len(stand_files)));stand_object=[]
...
[iraf.noao.imred.kpnoslit.continuum('dp_'+object_files[j][:-4]+'.ms.fits[0]',object_files[j][:-4]+'_co.fits',interact='no',low_reject = 2,high_reject = 2,niterate = 10,order=10,func='spline3') for j in range(len(object_files))]
